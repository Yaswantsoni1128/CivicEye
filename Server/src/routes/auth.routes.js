...
router.post("/signup", async (req, res) => {
  try {
    ...
    if (role && role !== "citizen")
      return res.status(403).json({ message: "Cannot signup as admin or worker" });
...
router.post("/create-worker", authMiddleware, allowRoles("admin"), async (req, res) => {
  try {
    ...
    const worker = await Worker.create({ name, phone, email, password: hashedPassword });
...
router.post("/create-admin", authMiddleware, allowRoles("admin"), async (req, res) => {
  try {
    const { name, phone, email, password } = req.body;
    ...
    const admin = await Admin.create({ name, phone, email, password: hashedPassword });
